...
def plot_tracks(fig, ax, ax2, track_list, meas_list, lidar_labels, lidar_labels_valid, 
    ...
    for track in track_list:
        if state == None or track.state == state: # plot e.g. only confirmed tracks
            ...
            # write track id and state as readable label
            state_short = {'confirmed': 'confirmed', 'tentative': 'tentative', 'initialized': 'init'}
            label_text = f"Car {track.id}\n[{state_short.get(track.state, track.state)}]"
            ax.text(float(-track.x[1]), float(track.x[0]) + 2.0, label_text,
                    fontsize=11, fontweight='bold', color='black', ha='center', va='bottom',
                    bbox=dict(facecolor='white', alpha=0.75, edgecolor=col, boxstyle='round,pad=0.3'))
...
def plot_rmse(manager, all_labels):
    ...
    if plot_empty:
        ...
        plt.xlabel('time [s]')
        plt.ylabel('RMSE [m]')
        save_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', 'results', 'rmse_plot.png')
...
def make_movie(path):
    # read only tracking frame plots (not RMSE/safety plots)
    images = sorted([img for img in os.listdir(path) if img.startswith("tracking") and img.endswith(".png")])
    if not images:
        print("No tracking frames found for movie generation.")
        return
    frame = cv2.imread(os.path.join(path, images[0]))
    height, width, layers = frame.shape

    # save with 10fps to result dir (MJPG: compressed but clear at full resolution)
    video = cv2.VideoWriter(os.path.join(path, 'my_tracking_results.avi'),
                            cv2.VideoWriter_fourcc(*'MJPG'), 10, (width, height))
    ...
    video.release()
    print(f"Tracking movie saved to: {os.path.join(path, 'my_tracking_results.avi')}")