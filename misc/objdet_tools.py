...
def project_detections_into_bev(bev_map, detections, configs, color=[]):
    for row in detections:
        ...
        corners_int2 = bev_corners.reshape(-1, 2).astype(int)
        cv2.line(bev_map, (int(corners_int2[0, 0]), int(corners_int2[0, 1])),
                 (int(corners_int2[3, 0]), int(corners_int2[3, 1])), (255, 255, 0), 2)
...
def save_detection_images(frame_idx, detections, bev_maps, image, object_labels,
                           object_labels_valid, camera_calibration, configs, save_dir):
    """Save BEV detection image and camera detection image to disk (headless-safe)."""
    import os

    # --- BEV image with detected boxes (blue) ---
    bev_map = (bev_maps.squeeze().permute(1, 2, 0).numpy() * 255).astype(np.uint8)
    bev_map = cv2.resize(bev_map, (configs.bev_width, configs.bev_height))
    project_detections_into_bev(bev_map, detections, configs, [0, 0, 255])   # detections = blue
    bev_map = cv2.rotate(bev_map, cv2.ROTATE_180)
    bev_path = os.path.join(save_dir, f'detection_bev_frame{frame_idx:03d}.png')
    cv2.imwrite(bev_path, bev_map)

    # --- Camera image with GT bounding boxes ---
    img_cam = image.copy()
    img_cam = project_labels_into_camera(camera_calibration, img_cam, object_labels,
                                          object_labels_valid, img_resize_factor=1.0)
    cam_path = os.path.join(save_dir, f'detection_cam_frame{frame_idx:03d}.png')
    cv2.imwrite(cam_path, img_cam)

    # --- Combined: camera on top, BEV below ---
    h_cam, w_cam = img_cam.shape[:2]
    scale = configs.output_width / w_cam
    cam_resized = cv2.resize(img_cam, (configs.output_width, int(h_cam * scale)))
    bev_resized = cv2.resize(bev_map, (configs.output_width, int(configs.bev_height * configs.output_width / configs.bev_width)))
    combined = np.vstack([cam_resized, bev_resized])
    comb_path = os.path.join(save_dir, f'detection_combined_frame{frame_idx:03d}.png')
    cv2.imwrite(comb_path, combined)
    print(f'  Saved detection images for frame {frame_idx}: BEV + camera + combined')
