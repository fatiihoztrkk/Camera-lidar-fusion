...
from student.safety_score import evaluate_safety, plot_safety_scores
 
##################
## Set parameters and perform initializations

## Select Waymo Open Dataset file and frame numbers
# Steps 4 & 5: Sequence 1
data_filename = 'training_segment-1005081002024129653_5313_150_5333_150_with_camera_labels.tfrecord' # Sequence 1
# data_filename = 'training_segment-10072231702153043603_5725_000_5745_000_with_camera_labels.tfrecord' # Sequence 2
# data_filename = 'training_segment-10963653239323173269_1924_000_1944_000_with_camera_labels.tfrecord' # Sequence 3

# Steps 4 & 5: all frames
...
# Steps 4 & 5: slightly narrowed for clearer visualization
configs_det.lim_y = [-20, 20]
...
# Step 5: camera-lidar fusion with tracking movie + detection PNGs
exec_detection = []
exec_tracking = ['perform_tracking', 'use_camera']
exec_visualization = ['show_tracks', 'load_image', 'make_tracking_movie', 'save_detection_images']
...
all_labels = []
det_performance_all = []
all_safety_assessments = []  # Step 6: collect safety scores
np.random.seed(0) # make random values predictable
if 'show_tracks' in exec_list:
    fig, (ax2, ax) = plt.subplots(1, 2, figsize=(20, 10)) # init track plot — bigger for readable labels
...
while True:
    try:
        ...
        # Save detection PNGs for selected frames
        if 'save_detection_images' in exec_list and cnt_frame in [10, 50, 100]:
            tools.save_detection_images(cnt_frame, detections, lidar_bev, image,
                                        frame.laser_labels, valid_label_flags,
                                        camera_calibration, configs_det, results_fullpath)
        ...
        if 'perform_tracking' in exec_list:
            ...
            for detection in detections:
                lidar.generate_measurement(cnt_frame, detection, meas_list_lidar)

            # preprocess camera detections (Step 5: camera-lidar fusion)
            meas_list_cam = []
            if 'use_camera' in exec_list:
                for label in frame.camera_labels[0].labels:
                    if(label.type == label_pb2.Label.Type.TYPE_VEHICLE):
                        box = label.box
                        z = [box.center_x, box.center_y, box.width, box.length]
                        camera.generate_measurement(cnt_frame, z, meas_list_cam)

            # Kalman prediction
            for track in manager.track_list:
                print('predict track', track.id)
                KF.predict(track)
                track.set_t((cnt_frame - 1) * params.dt)

            # associate and update all lidar measurements to all tracks
            association.associate_and_update(manager, meas_list_lidar, KF)

            # associate all camera measurements to all tracks (Step 5 only)
            if meas_list_cam:
            ...
            # Step 6: safety score evaluation
            frame_time = (cnt_frame - 1) * params.dt
            assessments = evaluate_safety(manager, frame_time)
            all_safety_assessments.extend(assessments)
...
## Step 6: Plot safety scores
if all_safety_assessments:
    plot_safety_scores(all_safety_assessments, results_fullpath)
