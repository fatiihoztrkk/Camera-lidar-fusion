...
def detect_objects(input_bev_maps, model, configs):
    ...
    if len(detections) > 0:
        ...
        for obj in detections:
            ...
            # bev_from_pcl uses bev_discret = x_diff/bev_height for BOTH axes,
            # so the inverse must also use x_diff for y and w (not lim_y diff)
            x = (obj_x / configs.bev_height) * x_diff
            y = (obj_y / configs.bev_width) * x_diff - x_diff / 2.0

            w = (obj_w / configs.bev_width) * x_diff
