...
class Track:
    ...
    def __init__(self, meas, id):
        ...
        # position uncertainty from rotated measurement noise; velocity uncertainty from params
...
class Trackmanagement:
    ...
    def manage_tracks(self, unassigned_tracks, unassigned_meas, meas_list):
        ...
        # decrease score only for unassigned tracks inside sensor FOV
        for i in unassigned_tracks:
            track = self.track_list[i]
            if len(meas_list) > 0 and meas_list[0].sensor.in_fov(track.x):
                track.score -= 1. / params.window

        # collect tracks to delete (avoid modifying list while iterating)
        tracks_to_delete = []
        for i in unassigned_tracks:
            track = self.track_list[i]
