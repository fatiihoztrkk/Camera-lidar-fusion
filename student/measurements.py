...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        if float(pos_sens[0]) <= 0:
            return False

        angle = np.arctan2(float(pos_sens[1]), float(pos_sens[0]))
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            i = self.c_i - self.f_i * float(pos_sens[1]) / float(pos_sens[0])
            j = self.c_j - self.f_j * float(pos_sens[2]) / float(pos_sens[0])
...
class Measurement:
    ...
    def __init__(self, num_frame, z, sensor):
        ...
        if sensor.name == 'lidar':
            ...
            self.z[0] = z[1]  # x  (z[0] is class id)
            self.z[1] = z[2]  # y
            self.z[2] = z[3]  # z height
